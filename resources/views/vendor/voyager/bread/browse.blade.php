...
@section('javascript')
    ...
    @if(preg_match('/_pages$/', $dataType->name))
        <script>
            @if(call_user_func([$dataType->model_name, 'count']))
                document.location.href = '{{ route('voyager.'.$dataType->slug.'.edit', 1) }}';
            @else
                document.location.href = '{{ route('voyager.'.$dataType->slug.'.create') }}';
            @endif
        </script>
    @endif
